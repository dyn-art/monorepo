...
export interface AnyCoreInputEvent {
	events: CoreInputEvent[];
}

export type AnyInputEvent =
	| ({ type: 'Core' } & AnyCoreInputEvent)
	| ({ type: 'Interaction' } & AnyInteractionInputEvent);

export interface AnyInteractionInputEvent {
	events: InteractionInputEvent[];
}
...
export type CoreInputEvent =
	| ({ type: 'EntityMoved' } & EntityMoved)
	| ({ type: 'EntitySetPosition' } & EntitySetPosition);

export interface CursorChangeEvent {
	cursor: CursorForFrontend;
}

export interface CursorDownOnComposition {
	position: Vec2;
}

export interface CursorDownOnEntity {
	entity: Entity;
	position: Vec2;
}

export interface CursorDownOnResizeHandle {
	initialBounds: XYWH;
	corner: number;
	rotationInRadians: number;
}

export interface CursorDownOnRotateHandle {
	corner: number;
	initialRotationInRadians: number;
}
...
export type CursorForFrontend =
	| { type: 'Default' }
	| { type: 'Resize'; rotationInDegrees: number }
	| { type: 'Rotate'; rotationInDegrees: number };

export interface CursorMovedOnComposition {
	position: Vec2;
}

export interface CursorUpOnComposition {
	position: Vec2;
}
 ...
 */
export interface DTIFComposition {
	/**
...
export type DTIFNode =
	| ({ type: 'Frame' } & FrameNodeBundle)
	| ({ type: 'Group' } & GroupNodeBundle)
	| ({ type: 'Rectangle' } & RectangleNodeBundle)
	| ({ type: 'Text' } & TextNodeBundle);
 ...
 */
export interface ElementAppended {
	parentId: number;
}
 ...
 */
export interface EllipseArcData {
	/**
...
export interface EntitySetPosition {
	entity: Entity;
	x: number;
	y: number;
}
 ...
 */
export interface FontWithContent {
	/**
...
export interface FrameNodeBundle {
	node?: Node;
	frame?: Frame;
	rectangleCornerMixin?: RectangleCornerMixin;
	children: ChildrenMixin;
	compositionMixin?: NodeCompositionMixin;
	relativeTransform: RelativeTransformMixin;
	dimension: DimensionMixin;
	blendMixin?: BlendMixin;
	fill?: FillMixin;
}
...
export interface GroupNodeBundle {
	node?: Node;
	group?: Group;
	children: ChildrenMixin;
	compositionMixin?: NodeCompositionMixin;
...
export type InteractionInputEvent =
	| ({ type: 'CursorDownOnEntity' } & CursorDownOnEntity)
	| ({ type: 'CursorMovedOnComposition' } & CursorMovedOnComposition)
	| { type: 'CursorEnteredComposition' }
	| { type: 'CursorExitedComposition' }
	| ({ type: 'CursorDownOnComposition' } & CursorDownOnComposition)
	| ({ type: 'CursorUpOnComposition' } & CursorUpOnComposition)
	| ({ type: 'CursorDownOnResizeHandle' } & CursorDownOnResizeHandle)
	| ({ type: 'CursorDownOnRotateHandle' } & CursorDownOnRotateHandle);

export interface InteractionModeChangeEvent {
	interactionMode: InteractionModeForFrontend;
}

export type InteractionModeForFrontend =
	| { type: 'None' }
	| { type: 'Pressing' }
	| { type: 'Translating' }
	| { type: 'Resizing' }
...
export type MixinChange =
	| ({ type: 'RectangleCorner' } & RectangleCornerMixin)
	| ({ type: 'Children' } & MixinChangeChildrenMixin)
	| ({ type: 'Dimension' } & DimensionMixin)
	| ({ type: 'RelativeTransform' } & MixinChangeRelativeTransformMixin)
	| ({ type: 'Composition' } & NodeCompositionMixin)
	| ({ type: 'Blend' } & BlendMixin)
	| ({ type: 'Path' } & PathMixin)
	| ({ type: 'Fill' } & FillMixin);
 ...
 */
export interface MixinChangeChildrenMixin {
	children: ChildrenMixin;
}

export interface MixinChangeRelativeTransformMixin {
	relativeTransform: RelativeTransformMixin;
}
 ...
 */
export interface NodeCompositionMixin {
	/**
...
export type OutputEvent =
	| ({ type: 'RenderUpdate' } & RenderUpdateEvent)
	| ({ type: 'TrackUpdate' } & TrackUpdateEvent)
	| ({ type: 'SelectionChange' } & SelectionChangeEvent)
	| ({ type: 'InteractionModeChange' } & InteractionModeChangeEvent)
	| ({ type: 'CursorChange' } & CursorChangeEvent);
...
export interface RectangleNodeBundle {
	node?: Node;
	recangle?: Rectangle;
	rectangleCornerMixin?: RectangleCornerMixin;
	compositionMixin?: NodeCompositionMixin;
...
export type RenderChange =
	| ({ type: 'ElementCreated' } & ElementCreated)
	| { type: 'ElementDeleted' }
	| ({ type: 'ElementAppended' } & ElementAppended)
	| ({ type: 'AttributeUpdated' } & AttributeUpdated)
	| ({ type: 'AttributeRemoved' } & AttributeRemoved)
	| ({ type: 'StyleUpdated' } & StyleUpdated)
	| ({ type: 'StyleRemoved' } & StyleRemoved);

export interface RenderUpdateEvent {
	id: number;
	updates: RenderChange[];
}
...
export type SVGAttribute =
	| { type: 'Id'; id: number }
	| { type: 'Width'; width: number; unit: SVGMeasurementUnit }
	| { type: 'Height'; height: number; unit: SVGMeasurementUnit }
	| { type: 'Opacity'; opacity: number }
	| { type: 'Transform'; transform: SVGTransformAttribute }
	| { type: 'D'; d: SVGPathCommand[] }
	| { type: 'ClipPath'; clipPath: number }
...
export type SVGPathCommand =
	...
	| { type: 'CurveTo'; cx1: number; cy1: number; cx2: number; cy2: number; x: number; y: number }
	| {
			type: 'ArcTo';
			rx: number;
			ry: number;
...
export type SVGStyle =
	| { type: 'Display'; display: SVGDisplayStyle }
	| { type: 'BlendMode'; blendMode: SVGBlendMode };

export interface SVGTransformAttribute {
	type: 'Matrix';
	a: number;
	b: number;
	c: number;
...
export interface TextNodeBundle {
	node?: Node;
	text: Text;
	compositionMixin?: NodeCompositionMixin;
	relativeTransform: RelativeTransformMixin;
