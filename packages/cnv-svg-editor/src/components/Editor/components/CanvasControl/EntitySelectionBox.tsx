...
export const EntitySelectionBox: React.FC<TProps> = (props) => {
	...
	const factor = useViewportFactor(canvas);
	...
	const interactionMode = useInteractionMode(canvas);
	...
	const handleOnResizeHandlePointerEvent = React.useCallback(
		(side: EHandleSide, eventType: 'Up' | 'Down', event: React.PointerEvent<SVGGElement>) => {
			...
			if (event.button === 0) {
				switch (eventType) {
					case 'Up':
						onResizeHandlePointerUp(canvas.builder.pointerEventToCnvPoint(event));
						break;
					case 'Down':
						onResizeHandlePointerDown(
							side,
	...
	const handleOnRotateHandlePointerEvent = React.useCallback(
		(side: EHandleSide, eventType: 'Up' | 'Down', event: React.PointerEvent<SVGGElement>) => {
			...
			if (event.button === 0) {
				switch (eventType) {
					case 'Up':
						onRotateHandlePointerUp(canvas.builder.pointerEventToCnvPoint(event));
						break;
					case 'Down':
		...
		[globalTransformData, canvas.builder, onRotateHandlePointerUp, onRotateHandlePointerDown]
	);
	...
	return (
		<g
			style={{
				transform: `translate(${factoredTranslation[0] - canvas.viewport.physicalPosition[0] * factor[0]}px, ${
					factoredTranslation[1] - canvas.viewport.physicalPosition[1] * factor[1]
...
interface TProps {
	...
	onResizeHandlePointerDown: (
		corner: EHandleSide,
		initialBounds: CNV.XYWH,
		rotationDeg: number
	) => void;
	onResizeHandlePointerUp: (position: CNV.Vec2) => void;
	onRotateHandlePointerDown: (side: EHandleSide, rotationDeg: number) => void;
	onRotateHandlePointerUp: (position: CNV.Vec2) => void;
}
