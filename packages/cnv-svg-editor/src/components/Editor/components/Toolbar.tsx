...
export const Toolbar: React.FC<TProps> = (props) => {
	const { canvas } = props;
	const interactionTool = useInteractionTool(canvas);
	...
	return (
		<aside className="absolute left-1/2 top-2 flex translate-x-[-50%] flex-col items-center justify-center rounded-lg bg-white px-2 py-2 shadow-md">
			<ToggleGroup
				onValueChange={(value) => {
					let tool: CNV.InteractionTool | null = null;
					...
					switch (value) {
						case 'Select':
							...
							break;
						case 'Shape':
							tool = { type: 'Shape', variant: shapeToolItem as CNV.ShapeVariant };
							break;
						default:
						// do nothing
					}
					...
					if (tool != null) {
						canvas.emitInputEvent('Interaction', { type: 'InteractionToolChanged', tool });
						canvas.update();
					}
				}}
				type="single"
							...
							onValueChange={(value) => {
								if (value != shapeToolItem) {
									...
									canvas.emitInputEvent('Interaction', {
										...
										tool: { type: 'Shape', variant: value as CNV.ShapeVariant }
									});
								}
							}}
							value={shapeToolItem}
