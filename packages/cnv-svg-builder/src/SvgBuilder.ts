...
import { toKeyCode, toMouseButton } from '@dyn/cnv-dtif';
import type {
...
export class SvgBuilder {
	...
	private readonly _cnv: () => Canvas; // TODO: Bad practice?
	...
	private _cursorInCnvBounds = false;

	constructor(canvas: Canvas, options: TSvgBuilderOptions = {}) {
		...
		canvas.watchOutputEvent('CanvasChange', (event) => {
			this.applyCanvasChange(event);
		});
		...
		if (this.isInteractive) {
			this._domElement.addEventListener('pointermove', (e) => {
				...
				this.canvas.emitInputEvent(
					'Interaction',
					{
						type: 'CursorMovedOnCanvas',
						position: this.pointerEventToCnvPoint(e)
					},
					false
				);
			});
			this._domElement.addEventListener('wheel', (e) => {
				...
				this.canvas.emitInputEvent(
					'Interaction',
					{
						type: 'MouseWheeledOnCanvas',
						position: this.clientWindowPointToCnvPoint([e.clientX, e.clientY]),
			...
			this._domElement.addEventListener('pointerenter', (e) => {
				e.preventDefault();
				if (!this._cursorInCnvBounds) {
					this.canvas.emitInputEvent(
						'Interaction',
						{
							type: 'CursorEnteredCanvas'
						},
						false
					);
					this._cursorInCnvBounds = true;
				}
			});
			this._domElement.addEventListener('pointerleave', (e) => {
				e.preventDefault();
				const cnvPoint = this.pointerEventToCnvPoint(e);
				// Check whether cursor actually left canvas
				// or whether its just on some UI layer like the selection box
				if (
					this._cursorInCnvBounds &&
					(cnvPoint[0] < 0 ||
						cnvPoint[0] > this.canvas.size[0] ||
						cnvPoint[1] < 0 ||
						cnvPoint[1] > this.canvas.size[1])
				) {
					this.canvas.emitInputEvent(
						'Interaction',
						{
							type: 'CursorExitedCanvas'
						},
						false
					);
					this._cursorInCnvBounds = false;
				}
			});
	...
	public applyCanvasChange(event: CanvasChangeOutputEvent): void {
	...
	public pointerEventToCnvPoint(event: PointerEvent | { clientX: number; clientY: number }): Vec2 {
		return this.clientWindowPointToCnvPoint([event.clientX, event.clientY]);
	}

	public clientWindowPointToCnvPoint(clientPoint: Vec2): Vec2 {
