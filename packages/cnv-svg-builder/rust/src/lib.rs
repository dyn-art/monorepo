...
use dyn_cnv_core::{
    resources::{
        canvas::CanvasRes,
        lua::{cnv_table::FrozenWorld, LuaRes},
    },
    CnvCorePlugin,
};
use dyn_cnv_dtif::DtifCanvas;
use dyn_cnv_interaction::CnvInteractionPlugin;
use dyn_cnv_lua::{freeze::Frozen, script::LuaScriptError};
use dyn_cnv_svg_builder::{
    events::SvgBuilderOutputEvent, svg::svg_bundle::SvgBundleVariant, CnvSvgBuilderPlugin,
};
use events::{SvgCnvInputEvent, SvgCnvOutputEvent};
use modules::watch::{resources::watched_entities::WatchedEntitiesRes, CnvWatchPlugin};
use std::sync::mpsc::{channel, Receiver};
use wasm_bindgen::{prelude::wasm_bindgen, JsValue};

#[wasm_bindgen]
pub struct SvgCnvHandle {
    app: App,
    svg_builder_output_event_receiver: Receiver<SvgBuilderOutputEvent>,
    output_event_receiver: Receiver<SvgCnvOutputEvent>,
}

#[wasm_bindgen]
impl SvgCnvHandle {
    pub fn create(js_dtif: JsValue, interactive: bool) -> Result<SvgCnvHandle, JsValue> {
        let mut dtif: DtifCanvas = serde_wasm_bindgen::from_value(js_dtif)?;
        let mut app = App::new();

        let (svg_builder_output_event_sender, svg_builder_output_event_receiver) =
            channel::<SvgBuilderOutputEvent>();
        let (output_event_sender, output_event_receiver) = channel::<SvgCnvOutputEvent>();
        ...
        if interactive {
            app.add_plugins(CnvInteractionPlugin);
        }
    ...
    pub fn update(&mut self, js_input_events: JsValue) -> Result<JsValue, JsValue> {
        let maybe_input_events: Result<Vec<SvgCnvInputEvent>, _> =
        ...
        if let Ok(input_events) = maybe_input_events {
            for input_event in input_events {
                match input_event {
                    SvgCnvInputEvent::Core { event } => {
                        event.send_into_world(&mut self.app.world);
                    }
                    SvgCnvInputEvent::Interaction { event } => {
                        event.send_into_world(&mut self.app.world);
                    }
                }
            }
        ...
        while let Ok(event) = self.svg_builder_output_event_receiver.try_recv() {
            match event {
                SvgBuilderOutputEvent::SvgElementChanges(event) => {
                    output_events.push(SvgCnvOutputEvent::SvgElementChange(event))
                }
                _ => {}
            }
        }
    ...
    pub fn to_string(&mut self) -> Option<String> {
        ...
        let cnv_res = self.app.world.get_resource::<CanvasRes>()?;
    ...
    pub fn temp_dev_log(&mut self) {
        #[cfg(feature = "tracing")]
        {
            ...
            use dyn_cnv_interaction::components::{Locked, Selected};
