...
mod cli {
    ...
    use dyn_cnv_svg_builder_api::*;

    // TODO: Remove once specta can detect specta::Type on its own in the repo
    #[derive(specta::Type)]
    struct SpectaExport {
        cnv_dtif: dyn_cnv_dtif::DtifCanvas,
        svg_cnv_input_event: dyn_cnv_svg_builder_api::events::SvgCnvInputEvent,
        svg_cnv_output_event: dyn_cnv_svg_builder_api::events::SvgCnvOutputEvent,
    }

    #[derive(Parser, Debug)]
    #[clap(name = "SVG Canvas CLI")]
    struct Cli {
        #[clap(subcommand)]
        pub cmd: SubCommand,
    }
