...
use dyn_arb_bundles::components::{marker::Root, mixins::HierarchyLevel, nodes::ArbNode};
...
pub fn remove_root_component_system(
    mut commands: Commands,
    query: Query<Entity, (With<Children>, With<Parent>, With<ArbNode>, With<Root>)>,
) {
    for entity in query.iter() {
...
pub fn update_hierarchy_levels(
    mut commands: Commands,
    changed_parents_query: Query<(Entity, &Parent), (With<ArbNode>, Changed<Parent>)>,
    children_query: Query<&Children, With<ArbNode>>,
    level_query: Query<&HierarchyLevel, With<ArbNode>>,
    parent_query: Query<&Parent, With<ArbNode>>,
) {
...
fn calculate_level_for_entity(entity: Entity, parent_query: &Query<&Parent, With<ArbNode>>) -> u8 {
