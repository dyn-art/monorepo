use crate::resources::{canvas::CanvasRes, referencer::ReferencerRes};
use bevy_ecs::{
    entity::Entity,
...
use dyn_cnv_asset::resources::AssetsRes;
use dyn_cnv_bundles::{
    components::{
        marker::{Removed, Root},
        mixins::{
        ...
        nodes::{
            CnvNode, EllipseCnvNode, FrameCnvNode, PolygonCnvNode, StarCnvNode, TextCnvNode,
        },
        paints::{CnvPaint, GradientCnvPaint, ImageCnvPaint, SolidCnvPaint},
        styles::{DropShadowCnvStyle, FillCnvStyle, StrokeCnvStyle},
    },
    events::{
        CreateAssetInputEvent, CreateNodeInputEvent, CreatePaintInputEvent, DeleteEntityInputEvent,
        FocusRootNodesInputEvent, MoveEntityInputEvent, UpdateCanvasSizeInputEvent,
...
pub fn update_canvas_size_input_system(
    ...
    mut event_reader: EventReader<UpdateCanvasSizeInputEvent>,
) {
    if let Some(event) = event_reader.read().last() {
        cnv_res.size = event.size;
        cnv_res.viewport.physical_size = event.size;
    }
}

pub fn update_canvas_viewport_input_system(
    mut cnv_res: ResMut<CanvasRes>,
    mut event_reader: EventReader<UpdateCanvasViewportInputEvent>,
) {
    if let Some(event) = event_reader.read().last() {
        cnv_res.viewport = event.viewport;
    }
}
...
pub fn focus_root_nodes_input_system(
    ...
    mut cnv_res: ResMut<CanvasRes>,
    query: Query<(&SizeMixin, &Transform), (With<Root>, With<CnvNode>)>,
) {
...
pub fn update_frame_node_input_system(
    ...
    mut query: Query<&mut FrameCnvNode>,
) {
    ...
    } in event_reader.read()
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut frame_cnv_node) = query.get_mut(entity) {
                if let Some(clip_content) = maybe_clip_content {
                    frame_cnv_node.clip_content = *clip_content;
                }
            }
        }
    }
...
pub fn update_ellipse_node_input_system(
    ...
    mut query: Query<&mut EllipseCnvNode>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut ellipse_cnv_node) = query.get_mut(entity) {
                if let Some(starting_angle) = maybe_starting_angle {
                    ellipse_cnv_node.arc_data.starting_angle = *starting_angle;
                }
                if let Some(ending_angle) = maybe_ending_angle {
                    ellipse_cnv_node.arc_data.ending_angle = *ending_angle;
                }
                if let Some(inner_radius_ratio) = maybe_inner_radius_ratio {
                    ellipse_cnv_node.arc_data.inner_radius_ratio = *inner_radius_ratio;
                }
            }
        }
    }
...
pub fn update_star_node_input_system(
    ...
    mut query: Query<&mut StarCnvNode>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut star_cnv_node) = query.get_mut(entity) {
                if let Some(point_count) = maybe_point_count {
                    star_cnv_node.point_count = *point_count;
                }
                if let Some(inner_radius_ratio) = maybe_inner_radius_ratio {
                    star_cnv_node.inner_radius_ratio = *inner_radius_ratio;
                }
            }
        }
    }
...
pub fn update_polygon_node_input_system(
    ...
    mut query: Query<&mut PolygonCnvNode>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut polygon_cnv_node) = query.get_mut(entity) {
                if let Some(point_count) = maybe_point_count {
                    polygon_cnv_node.point_count = *point_count;
                }
            }
        }
    }
...
pub fn update_text_node_input_system(
    ...
    mut query: Query<&mut TextCnvNode>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut text_cnv_node) = query.get_mut(entity) {
                if let Some(text) = maybe_text {
                    text_cnv_node.text = text.clone();
                }
                if let Some(attributes) = maybe_attributes {
                    text_cnv_node.attributes = SmallVec::from_vec(attributes.clone());
                }
                if let Some(line_wrap) = maybe_line_wrap {
                    text_cnv_node.line_wrap = *line_wrap;
                }
                if let Some(horizontal_text_alignment) = maybe_horizontal_text_alignment {
                    text_cnv_node.horizontal_text_alignment = *horizontal_text_alignment;
                }
                if let Some(vertical_text_alignment) = maybe_vertical_text_alignment {
                    text_cnv_node.vertical_text_alignment = *vertical_text_alignment;
                }
                if let Some(sizing_mode) = maybe_sizing_mode {
                    text_cnv_node.sizing_mode = *sizing_mode;
                }
            }
        }
    }
...
pub fn update_fill_style_input_system(
    ...
    mut style_query: Query<&mut PaintChildMixin, With<FillCnvStyle>>,
    mut paint_query: Query<&mut PaintParentMixin, With<CnvPaint>>,
) {
...
pub fn update_storke_style_input_system(
    ...
    mut style_query: Query<(&mut StrokeCnvStyle, &mut PaintChildMixin), With<StrokeCnvStyle>>,
    mut paint_query: Query<&mut PaintParentMixin, With<CnvPaint>>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok((mut stroke_cnv_style, mut paint_child_mixin)) = style_query.get_mut(entity) {
                if let Some(paint_entity) = maybe_paint_id.as_ref().and_then(|paint_id| {
                    paint_id.get_entity(referencer_res.get_reference_id_to_entity_map())
                }) {
                    // Dissolve old style-to-paint relationship
                ...
                if let Some(width) = maybe_width {
                    stroke_cnv_style.stroke.width = width.to_pt();
                }
            }
        }
    }
...
pub fn update_drop_shadow_style_input_system(
    ...
    mut query: Query<&mut DropShadowCnvStyle, With<StrokeCnvStyle>>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut drop_shadow_cnv_style) = query.get_mut(entity) {
                if let Some(color) = maybe_color {
                    drop_shadow_cnv_style.color = *color;
                }
                if let Some(position) = maybe_position {
                    drop_shadow_cnv_style.position = *position;
                }
                if let Some(spread) = maybe_spread {
                    drop_shadow_cnv_style.spread = *spread;
                }
                if let Some(blur) = maybe_blur {
                    drop_shadow_cnv_style.blur = *blur;
                }
            }
        }
    }
...
pub fn update_solid_paint_input_system(
    ...
    mut query: Query<&mut SolidCnvPaint>,
) {
    for UpdateSolidPaintInputEvent { id, color } in event_reader.read() {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut solid_cnv_paint) = query.get_mut(entity) {
                solid_cnv_paint.color = *color;
            }
        }
    }
}
...
pub fn update_image_paint_input_system(
    ...
    mut query: Query<(&mut ImageCnvPaint, &mut ImageAssetMixin)>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok((mut image_cnv_paint, mut image_asset_mixin)) = query.get_mut(entity) {
                if let Some(scale_mode) = maybe_scale_mode {
                    image_cnv_paint.scale_mode = *scale_mode;
                }
...
pub fn update_gradient_paint_input_system(
    ...
    mut query: Query<&mut GradientCnvPaint>,
) {
    ...
    {
        if let Some(entity) = id.get_entity(referencer_res.get_reference_id_to_entity_map()) {
            if let Ok(mut gradient_cnv_paint) = query.get_mut(entity) {
                if let Some(variant) = maybe_variant {
                    gradient_cnv_paint.variant = *variant;
                }
                if let Some(stops) = maybe_stops {
                    gradient_cnv_paint.stops = SmallVec::from_vec(stops.clone());
                }
            }
        }
    }
...
pub fn update_entity_opacity_input_system(
    ...
    paint_query: Query<&PaintParentMixin, With<CnvPaint>>,
) {
...
pub fn register_lua_script_input_system(
    ...
    mut event_reader: EventReader<dyn_cnv_bundles::events::RegisterLuaScriptInputEvent>,
) {
    use dyn_cnv_bundles::events::RegisterLuaScriptInputEvent;
...
pub fn execute_lua_script_input_system(
    ...
    system_state: &mut SystemState<(
        ...
        EventReader<dyn_cnv_bundles::events::ExecuteLuaScriptInputEvent>,
    )>,
) {
    use crate::resources::lua::{cnv_table::FrozenWorld, LuaRes};
    use dyn_cnv_bundles::events::ExecuteLuaScriptInputEvent;
    use dyn_cnv_lua::freeze::Frozen;
