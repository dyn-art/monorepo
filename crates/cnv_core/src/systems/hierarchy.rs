...
use dyn_cnv_bundles::components::{marker::Root, mixins::HierarchyLevel, nodes::CnvNode};
...
pub fn remove_root_component_system(
    mut commands: Commands,
    query: Query<Entity, (With<Children>, With<Parent>, With<CnvNode>, With<Root>)>,
) {
    for entity in query.iter() {
...
pub fn update_hierarchy_levels(
    mut commands: Commands,
    changed_parents_query: Query<(Entity, &Parent), (With<CnvNode>, Changed<Parent>)>,
    children_query: Query<&Children, With<CnvNode>>,
    level_query: Query<&HierarchyLevel, With<CnvNode>>,
    parent_query: Query<&Parent, With<CnvNode>>,
) {
...
fn calculate_level_for_entity(entity: Entity, parent_query: &Query<&Parent, With<CnvNode>>) -> u8 {
