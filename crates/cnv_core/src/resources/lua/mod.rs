...
use cnv_table::{load_cnv_table_global, FrozenWorld};
use dyn_cnv_bundles::reference_id::ReferenceId;
use dyn_cnv_lua::{
...
impl LuaRes {
    ...
    pub fn setup_lua(
        ...
        if let Some(LuaScript { source }) = self.scripts.get(id) {
            ...
            lua.enter(|ctx| {
                load_cnv_table_global(ctx, frozen_world);
            });
...
mod tests {
    ...
    use dyn_cnv_bundles::{
        events::{
            ...
            UpdateCanvasSizeInputEvent, UpdateEntityTransformInputEvent, UpdateTextNodeInputEvent,
        },
    ...
    fn e2e() {
        ...
        let code = r#"
            ...
            cnv.sendEvents({ update_canvas_size_event, update_entity_transform_event })
    ...
    fn update_canvas_size_input_system(mut event_reader: EventReader<UpdateCanvasSizeInputEvent>) {
