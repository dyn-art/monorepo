...
use dyn_cnv_bundles::events::{CoreInputEvent, InputEvent};
use dyn_cnv_lua::{
...
pub fn load_cnv_table_global<'gc>(ctx: Context<'gc>, frozen_world: FrozenWorld) {
    let cnv_table = create_cnv_table(ctx, frozen_world);
    ctx.set_global("cnv", cnv_table).unwrap();
}

fn create_cnv_table<'gc>(ctx: Context<'gc>, frozen_world: FrozenWorld) -> Table<'gc> {
    let cnv_table = Table::new(&ctx);
