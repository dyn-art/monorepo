use crate::{
    components::Selected,
    events::CursorUpOnCnvInputEvent,
    input::mouse::{
...
pub fn cursor_up_on_cnv_input_system(
    mut event_reader: EventReader<CursorUpOnCnvInputEvent>,
    ...
    for event in event_reader.read() {
        log::info!("[cursor_up_on_cnv_input_system] {:?}", event.button);
        mouse_button_on_cnv_input_res.release(event.button);
...
pub fn cursor_up_on_cnv_system(
    mut commands: Commands,
    mut cnv_interaction_res: ResMut<CnvInteractionRes>,
    mouse_button_on_cnv_input_res: Res<MouseButtonButtonInputRes>,
    ...
    if mouse_button_on_cnv_input_res.was_any_just_released() {
        match cnv_interaction_res.interaction_mode {
            InteractionMode::Inserting {
                ...
                if let Some(entity) = maybe_entity {
                    ...
                    cnv_interaction_res.interaction_tool = InteractionTool::Select;
                }
            }
            _ => {}
        };
