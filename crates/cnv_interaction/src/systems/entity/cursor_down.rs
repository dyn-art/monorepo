...
pub fn cursor_down_on_entity_system(
    ...
    unselected_node_query: Query<
        ...
        (
            With<CnvNode>,
            Or<(
                (Without<Root>, With<FrameCnvNode>),
                (With<Root>, Without<FrameCnvNode>),
                (Without<Root>, Without<FrameCnvNode>),
    ...
    } in selection_candidates.iter().copied()
    {
        if preselect {
            ...
            cnv_interaction_res.interaction_mode = InteractionMode::Translating {
