...
impl DTIFProcessor {
    ...
    pub fn process_node(
        ...
        dtif.nodes.get(node_eid).map(|dtif_node| {
            ...
            self.handle_fill(world, dtif, dtif_node, node_entity);
            self.handle_children(world, dtif, dtif_node, node_entity);

            return node_entity;
        })
    }

    /// Handles the fill mixin of a DTIF node, if present.
    fn handle_fill(
        &self,
        world: &mut World,
        dtif: &DTIFComposition,
        ...
        | DTIFNode::Rectangle(RectangleNodeBundle { fill_mixin, .. }) = dtif_node
        {
            ...
            let new_fill_mixin = FillMixin {
                paints: fill_mixin
                    .paints
                    .iter()
                    .filter_map(|paint_id| {
                        let paint_eid = DTIFProcessor::entity_to_eid(paint_id);
                        dtif.paints
                            .get(&paint_eid)
            ...
            world.entity_mut(node_entity).insert(new_fill_mixin);
        }
    }

    /// Handles the children of a DTIF node, if any, by processing each child and establishing
