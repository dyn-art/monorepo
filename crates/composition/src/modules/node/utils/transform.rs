...
pub fn extract_transform_data(transform: &Mat3) -> (f32, Vec2, Vec2) {
...
///
/// Arguments:
/// * `transform`: A `Mat3` matrix representing the initial transformation
///    (including position, rotation, skew) of the object in 2D space.
/// * `angle_in_radians`: The angle of rotation in radians.
/// * `relative_pivot_point`: A `Vec2` vector representing the pivot point for rotation,
...
pub fn apply_rotation(transform: Mat3, angle_in_radians: f32, relative_pivot_point: Vec2) -> Mat3 {
    // Translation matrices for moving to the pivot point and back to the origin
    let translate_to_pivot = Mat3::from_translation(relative_pivot_point);
    let translate_to_origin = Mat3::from_translation(-relative_pivot_point);
...
pub fn set_rotation(transform: Mat3, angle_in_radians: f32, pivot_point: Vec2) -> Mat3 {
    let (current_angle, _, _) = extract_transform_data(&transform);
    let reset_rotation_transform = apply_rotation(transform, -current_angle, pivot_point);
    return apply_rotation(reset_rotation_transform, angle_in_radians, pivot_point);
}

pub fn apply_transform_to_point(transform: Mat3, point: Vec2) -> Vec2 {
    let transformed_point = transform * Vec3::new(point.x, point.y, 1.0);
    return Vec2::new(transformed_point.x, transformed_point.y);
}
