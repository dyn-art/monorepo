...
pub fn cursor_down_on_entity_system(
    ...
    unselected_node_query: Query<
        ...
        (
            With<ArbNode>,
            Or<(
                (Without<Root>, With<FrameArbNode>),
                (With<Root>, Without<FrameArbNode>),
                (Without<Root>, Without<FrameArbNode>),
    ...
    } in selection_candidates.iter().copied()
    {
        if preselect {
            ...
            arb_interaction_res.interaction_mode = InteractionMode::Translating {
